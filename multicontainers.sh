...
docker run -v$PWD:/in -t ubuntu sh -c "stty -onlcr && /in/src/genseq.sh" > temp/sequence.txt
docker run -v$PWD:/in -t ubuntu sh -c "cat /in/temp/sequence.txt | /in/src/pick5th.sh"